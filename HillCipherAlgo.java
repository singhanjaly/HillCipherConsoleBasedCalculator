...
public class HillCipherAlgo {
	...
	public static int[][] getKeyMatrix(){
		...
		if(sq!=(long)sq) {
			System.out.println("Cannot form a square matrix. Please enter a valid key.");
		}
		int len=(int)sq;
	...
	//check whether key matrix is valid or not
	private static void isValidMatrix(int[][] keyMatrix) {
		int det=keyMatrix[0][0]*keyMatrix[1][1]-keyMatrix[0][1]*keyMatrix[1][0];
		//if det=0 (that is determinant), throw exception and terminate
		if(det==0) {
			throw new java.lang.Error("Determinants equals to zero, invalid key matrix!");
		}
	}
	
	//check whether the reverse of the matrix is valid or not
	//-----------------------------------------------------------------------------------------------------------------
	/******************************************************************************************************************/
	private static void isValidReverseMatrix(int[][] keyMatrix, int[][] reverseMatrix) {
		int[][] product=new int[2][2];
		//finding the product matrix of key matrix times reverse key matrix
	...
	// calculate the reverse key matrix
	private static int[][] reverseMatrix(int[][] keyMatrix){
		...
		//find the factor for which is true that (factor*det=1 mod 26)
	...
	//echo the result of encryption /decryption
	...
	public static void encrypt(String phrase, boolean alphaZero) {
		int i;
		int adder=alphaZero?1:0; //for calculations depending on alphabet
		int[][] keyMatrix;
		...
		// Get 2x2 key matrix from sc
		keyMatrix=getKeyMatrix();
		
		//check if matrix is valid
		isValidMatrix(keyMatrix);
		//convert characters to numbers according to their place in ascii table
		//minus 64 positions(A=65 in ASCII table)
